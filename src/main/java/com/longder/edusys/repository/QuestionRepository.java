...
public interface QuestionRepository {
    ...
    @Insert("INSERT INTO QUESTION(content_, type_, score_, answer_, choice_A_, choice_B_, choice_C_, choice_D_) " +
            "VALUES(#{content},#{type.name},#{score},#{answer},#{choiceA},#{choiceB},#{choiceC},#{choiceD})")
    ...
    @Select("SELECT Q.id_ AS q_id,Q.content_ as q_content,Q.* " +
            "    FROM QUESTION Q")
    ...
    @ResultMap("com.longder.edusys.repository.QuestionRepository.QuestionResultMap")
    @Select("SELECT Q.id_ AS q_id,Q.content_ as q_content,Q.* " +
            "    FROM QUESTION Q WHERE Q.id_ = #{questionId}")
